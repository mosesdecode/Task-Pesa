...
import { processActivationSuccess } from '../lib/activation';
...
async function runTests() {
  ...
  try {
    ...
    assert(Boolean(userA.referralCode && userA.referralCode.startsWith('TM-')), 'User A referral code format', userA.referralCode || '');
    assert(userA.wallet?.availableBalance === 0, 'User A initial wallet balance is KES 0');
    ...
    assert(userAWalletBeforePay?.availableBalance === 0, 'User A has received KES 0 before activation payment');
    ...
    // Execute activation processing engine
    const activationResult = await processActivationSuccess({
      checkoutRequestId: depositB.checkoutRequestId || undefined,
    ...
    assert(Boolean(paymentEntry && paymentEntry.amount === 200.0), 'Ledger recorded KES 200 ACTIVATION_PAYMENT');
    assert(Boolean(adminEntry && adminEntry.amount === 100.0), 'Ledger recorded KES 100 ACTIVATION_ADMIN_EARNING');
    assert(Boolean(referralEntry && referralEntry.amount === 100.0), 'Ledger recorded KES 100 REFERRAL_REWARD');

    // Accounting check: Admin (100) + Referral (100) === Payment (200)
    const ledgerSumB = (adminEntry?.amount || 0) + (referralEntry?.amount || 0);
    assert(ledgerSumB === (paymentEntry?.amount || 0), '100% Balanced Accounting: Admin (100) + Referral (100) == 200', `Sum: ${ledgerSumB}`);
    ...
    assert(userAWalletAfterPay?.availableBalance === 100.0, 'User A wallet credited exactly KES 100', `Balance: ${userAWalletAfterPay?.availableBalance}`);
    ...
    assert(Boolean(userATransaction && userATransaction.amount === 100.0), 'User A wallet transaction exists with type REFERRAL_REWARD');
    ...
    const activationResultC = await processActivationSuccess({
      checkoutRequestId: depositC.checkoutRequestId || undefined,
    ...
    assert(Boolean(paymentEntryC && paymentEntryC.amount === 200.0), 'User C recorded KES 200 ACTIVATION_PAYMENT');
    assert(Boolean(adminEntryC && adminEntryC.amount === 100.0), 'User C recorded KES 100 ACTIVATION_ADMIN_EARNING');
    assert(Boolean(retainedEntryC && retainedEntryC.amount === 100.0), 'User C recorded KES 100 PLATFORM_RETAINED_AMOUNT');
    ...
    const ledgerSumC = (adminEntryC?.amount || 0) + (retainedEntryC?.amount || 0);
    assert(ledgerSumC === (paymentEntryC?.amount || 0), '100% Balanced Accounting: Admin (100) + Retained (100) == 200', `Sum: ${ledgerSumC}`);
    ...
    const userD = await prisma.user.create({
      data: {
        ...
        mpesaNumber: `254744${timestamp.toString().slice(-6)}`,
    ...
    assert(userAWallet?.availableBalance === 100.0, 'Wallet balance matches available funds');
    ...
    assert((adminEarningsAgg._count.id || 0) >= 2, 'Admin earnings recorded for both tested activations');
    ...
    await prisma.adminAuditLog.deleteMany({
      where: {
        OR: [
          { adminId: { in: [userA.id, userB.id, userC.id, userD.id] } },
          { targetId: { in: [userA.id, userB.id, userC.id, userD.id] } },
        ],
      },
    });
